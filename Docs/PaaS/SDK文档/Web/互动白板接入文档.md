...
| 参数 | 类型   | 是否必填 | 说明    |
| ---- | ------ | -------- | ------- |
| file.title  | String | 是       | 文件名 |
| file.resolution  | String | 是       | 文件的分辨率 |
| file.url  | String | 是       | 转码后的url |
| file.pages  | Number | 是       | 文件总页数 |
...
| 回调参数 | 类型   | 说明                                                         |
| -------- | ------ | ------------------------------------------------------------ |
| code     | Number | 警告回调类型 <br> 1：接收到其他端的同步数据解析错误 <br> 3：上传的文件已经存在，不能重复上传 <br> 5：历史数据同步完成前，禁止操作白板 |
| msg      | String | 错误信息                                                     |
