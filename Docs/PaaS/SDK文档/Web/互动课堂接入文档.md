...
<!-- 白板SDK -->
<script src="https://tic-res-1259648581.file.myqcloud.com/board/2.3.0/TEduBoard.min.js"></script>
<!-- TIC SDK -->
<script src="https://tic-res-1259648581.file.myqcloud.com/tic/2.2.1/TIC.min.js"></script>
```

> 其中TIC.min.js可根据开源源码自行编译。
...
| 参数 | 类型 |  说明 |
| ---------| ---- | --------- |
| module | Number | 0： IM模块   1: TRTC模块 2： 白板模块 |
| code | Number | 错误码， 0： 正确<br/>[IM模块错误码文档](https://cloud.tencent.com/document/product/269/1671) <br/>[TRTC模块错误码文档](https://cloud.tencent.com/document/product/647/34342)<br/>[白板模块错误码](./互动白板接入文档.md) |
| desc | String | 错误描述 |
...
webrtcOption | Number | 是 | webrtcOption参数
boardOption | Number | 是 | boardOption参数
callback | Function | 是 | 回调
...
|recordId | Number | 否 | 自动录制时业务自定义 ID、Int32，录制回调时给到用户 <br/> <font color="red">注意：</font>如果任意Web端或小程序端用户传了recordId，必须保证其他Web端和小程序传递的值一致。|
...
| textStyle               |  Number  | 否 | 0 | 文字输入功能的默认文字样式<br/> 0：常规；1：粗体；2.斜体；3.粗斜体，默认值 0
| textSize                |  Number  | 否 | 320 | 文字输入功能的字体，实际像素值取值(size * 白板的高度 / 10000)px        
| textColor               |  String  | 否 | #000000  | 文字输入功能的颜色，支持 hex，rgba格式，如 #ffffff，rgba(255, 255, 255, 0.3)           
...
| progressEnable | Boolean | 否 | false | 是否启用SDK内置loading图标 |   
...
TIC 中只封装了 WebRTC 初始化和进房操作，您可以通过获取 WebRTC 实例做更多高级操作，更多 WebRTC 相关内容，请移步到[腾讯云TRTC文档](https://cloud.tencent.com/document/product/647/17249)
...
白板相关操作可直接调用 TIC 提供的获取白板实例接口来执行，TIC 不做任何封装，详情请参见 [白板 SDK接口文档](互动白板接入文档.md)。
