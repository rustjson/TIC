...
参数ID|参数类型|解释|获取方式
:--:|:--:|:--:|:--
company_id|int|机构码：获取机构的信息(机构名称，应用icon等)的唯一标识|申请Saas服务邮件获取。[参考邮件获取方式]()
class_id|int|课堂id：获取课堂信息的唯一标识|通过云API创建课堂获取。[参考云API]()
user_id|string|用户帐号|通过云API创建用户获取[参考云API]()
user_token|string|用户签名|通过云API创建用户获取[参考云API]()
