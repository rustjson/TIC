 ...
 *  8.5 设置本地视频的二次加工回调
 *
 * @note 设置之后，SDK会在送编码器之前将该帧视频画面回调出来，供您进行二次图像加工（比如叠加字幕，角标等等），之后SDK会将处理后的画面送入编码器
 * @note 注意：由于该回调过程是同步的，所以请注意控制二次加工的时间，不宜超过 20ms，否则会导致画面卡顿
 * @note setVideoPostProcessDelegate(null)可以停止回调
 *
 * @param delegate    自定义渲染回调
 * @param pixelFormat 指定回调的像素格式, 目前仅支持TRTCVideoPixelFormat_NV12
 * @param bufferType  回调数据类型，目前仅支持TRTCVideoBufferType_PixelBuffer
 * @return 0:成功 <0 错误
 */
- (int)setVideoPostProcessDelegate:(id<TRTCVideoPostProcessDelegate>)delegate pixelFormat:(TRTCVideoPixelFormat)pixelFormat bufferType:(TRTCVideoBufferType)bufferType;
