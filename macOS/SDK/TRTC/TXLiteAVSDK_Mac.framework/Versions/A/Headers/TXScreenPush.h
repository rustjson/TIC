...
//
//  Created by kennethmiao on 2019/3/19.
...
///推流器
@interface TXScreenPush : NSObject
///推流参数配置
///@see TXScreenPushConfig
@property(nonatomic, copy) TXScreenPushConfig *config;
...
/// 获取窗口列表
@property (class, readonly, nonatomic) NSArray<TXScreenCaptureSourceInfo*>* windowList;

/// 获取显示器列表
@property (class, readonly, nonatomic) NSArray<TXScreenCaptureSourceInfo*>* screenList;

/**
 初始化推流器
 @param config 推流参数配置
 */
- (id)initWithConfig:(TXScreenPushConfig *)config;
...
/**
 ...
 当从前台切到后台的时候，调用pausePush会推配置里设置的图片(TXScreenPushConfig.pauseImg)
 
 pauseImg  设置后台推流的默认图片，不设置为默认黑色背景
 pauseFps  设置后台推流帧率，最小值为5，最大值为20，默认10
 pauseTime 设置后台推流持续时长，单位秒，默认300秒
 具体使用方式请参考demo里面的示例
 @note 暂停推流，后台视频发送TXScreenPushConfig里面设置的图像，音频会继续录制声音发送, 如果不需要录制声音，需要再调下setMute接口
 */
- (void)pausePush;
 ...
 *
 */
- (void)selectScreenCaptureTarget:(TXScreenCaptureSourceInfo *)screenSource
                             rect:(CGRect)rect
...
/**
 ...
 @note autoSampleBufferSize优先级高于sampleBufferSize @see TXScreenPushConfig
