...
typedef NS_ENUM(NSInteger, TXScreenCaptureSourceType) {
    TXScreenCaptureSourceTypeUnknown      =   -1,
    TXScreenCaptureSourceTypeWindow       =    0,    ///< 该分享目标是某一个Mac窗口
    TXScreenCaptureSourceTypeScreen       =    1,    ///< 该分享目标是整个Mac桌面
};


@interface TXScreenCaptureSourceInfo : NSObject
/// 采集源是整个屏幕？还是某个窗口？
@property (assign, nonatomic) TXScreenCaptureSourceType type;
/// 采集源ID
...
@interface TXScreenPushConfig : NSObject
