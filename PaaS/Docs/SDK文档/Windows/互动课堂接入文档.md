...
（3）生成前事件:
项目【属性】-->【生成事件】-->【生成前事件】-->【命令行】-->在这里编写脚本，将sdk依赖的文件拷贝到项目输出目录;
例如:
```
...
请将进房参数`TICClassroomOption`的`classScene`设置为`TIC_CLASS_SCENE_LIVE`，并指定老师进房角色`roleType`为`TIC_ROLE_TYPE_ANCHOR`，学生进房角色`roleType`为`TIC_ROLE_TYPE_AUDIENCE`。
