...
```
teduBoard.switchFile(fid, pageIndex, stepIndex)
```

| 参数 |  类型   | 是否必填 | 说明 |
| ----------- | -----------| ---- | ------------------ |
| fid |   String | 是 | 要切换到的文件 ID，文件 ID 为必填项，为空字符串将导致文件切换失败 |
| pageIndex |   Number | 否 | 翻到指定页，不填表示以历史数据为指定页 |
| stepIndex |   Number | 否 | 翻到指定步，不填表示以历史数据为指定步 |
...
| transcodeConfig  | Object | 是 | 转码参数 |
| transcodeConfig.minResolution | String | 是 |最小分辨率，如960x540，中间分隔符为x  |
